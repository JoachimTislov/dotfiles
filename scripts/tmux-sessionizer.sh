...
# In short, is lists all the directories in your TS_SEARCH_PATHS and single directories
# in SINGLE_DIRS, then it lists all your tmux sessions. These dirs can be selected via fzf.
# If the tmux session already exists, it switches to it. If not, it creates a new tmux session.
...
TS_DIRS=(~/dotfiles ~/notes ~/.config ~/.config/nvim)
